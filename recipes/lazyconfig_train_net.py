...
def do_train(args, cfg):
    ...
    trainer = (
        AMPTrainer(model, train_loader, optim, grad_clipper=cfg.train.amp.grad_clipper)
...
def main(args):
    ...
    if args.eval_only:
        ...
        # print(f'Allocated memory after load model: {torch.cuda.memory_allocated() / (1024 ** 2)}MB')
        # print(f'Reserved memory after load model: {torch.cuda.memory_reserved() / (1024 ** 2)}MB')

        # print(f'model.parameters: {total_num}')
        # from torchstat import stat
        # stat(model.backbone.to('cpu'), (16, 256, 256))

        do_test(cfg, model)
        # print(f'Allocated memory after test: {torch.cuda.memory_allocated() / (1024 ** 2)}MB')
        # print(f'Reserved memory after test: {torch.cuda.memory_reserved() / (1024 ** 2)}MB')
    else:
        do_train(args, cfg)
